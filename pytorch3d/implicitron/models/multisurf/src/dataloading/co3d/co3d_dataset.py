...
class Co3dDataset(torch.utils.data.Dataset):
    ...
    def _get_pytorch3d_camera(self, entry, scale, clamp_bbox_xyxy):
        ...
        # print("principal_points", principal_point)
        # print("focal_length", focal_length)
        # print("clamp_bbox_xyxy",clamp_bbox_xyxy)
        # print("image.size", entry.image.size)
        ...
        # print("principal_point_px", principal_point_px)
        ...
        # print('scale', scale)
        # print('half_image_size_wh_output',half_image_size_wh_output)
        # print(torch.tensor(entry.viewpoint.R, dtype=torch.float).shape)
    ...
    #     mask[:, 0 : imre.shape[1] - 1, 0 : imre.shape[2] - 1] = 1.0
    #     '''
    #     imre_ = imre[:, 0:self.image_height, 0:self.image_width]
    #     print("image", image.shape)
    #     print("imre",imre.shape)
    #     print("imre_", imre_.shape)
    ...
    def _resize_image(self, image, mode="bilinear"):
        ...
        mask[:, 0 : imre.shape[1] - 1, 0 : imre.shape[2] - 1] = 1.0
        '''
        imre_ = imre[:, :self.image_height, :self.image_width]
        mask = torch.ones(1, self.image_height, self.image_width)
        return imre_, minscale, mask
...
def _get_bbox_from_mask(mask, thr, decrease_quant=0.05):
    ...
    if w > h:
        ...
        x1  = x1 + h - w
        return x0, y0, x1 - x0, y1 - y0
# def _get_bbox_from_mask(self, mask, thr, decrease_quant=0.05):
#         # bbox in xywh
#     masks_for_box = np.zeros_like(mask)
#     while masks_for_box.sum() <= 1.0:
#         masks_for_box = (mask > thr).astype(np.float32)
