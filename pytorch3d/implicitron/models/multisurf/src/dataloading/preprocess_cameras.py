...
def get_Ps(cameras,number_of_cameras):
    ...
    for i in range(0, number_of_cameras):
        P = cameras['world_mat_%d' % i][:3, :].astype(np.float)
        Ps.append(P)
    return np.array(Ps)
...
def get_min_max_d(curx, cury, P_j, silhouette_j, P_0, Fj0, j):
    ...
    relevant_matching_points_1 = silhouette_j[:, dists < 7]
...
def get_all_mask_points(masks_dir):
    mask_paths = sorted(re_utils.glob_imgs(masks_dir))
    mask_points_all=[]
    mask_ims = []
...
def get_normalization(source_dir, use_linear_init=False):
    ...
    masks_dir='{0}/resize_masks'.format(source_dir)
